...
int main( int argc, const char** argv )
{
	...
	for( int i = 0; i < accepted.size(); i++ ) //Exit loop early when all ground truths seen
	{
		...
		else cout << "Detected dartboard " << i+1 << " " << accepted[i]  << "doesn't match a ground truth dartboard" << endl;
	}
	float prec = (float) truePos / accepted.size(); //ratio of dartboards found correctly, to dartboards detected in image
